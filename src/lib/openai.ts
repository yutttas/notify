  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'あなたは夫婦関係の専門カウンセラーです。お二人の回答の違いから、認識のずれているポイントを指摘し、建設的なアドバイスを提供します。ただし、あくまでAIによる推測であることを踏まえ、断定的な表現は避け、「〜のようです」「〜かもしれません」「〜の可能性があります」など、柔らかく抽象度の高い表現を使用してください。点数などの数値は一切言及せず、温かく前向きなアドバイスを心がけてください。',
        },
        {
          role: 'user',
          content: prompt,
...
【表現の指示】断定的な表現は避け、「〜のようです」「〜かもしれません」「〜の可能性があります」など、柔らかく曖昧性のある表現を使用してください。`
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: '夫婦関係の専門カウンセラー。温かく前向きなメッセージを伝える。ただし、あくまでAIによる推測であることを踏まえ、断定的な表現は避け、「〜のようです」「〜かもしれません」「〜の可能性があります」など、柔らかく抽象度の高い表現を使用してください。',
        },
        {
          role: 'user',
          content: prompt,
